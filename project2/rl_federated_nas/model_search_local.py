...
class Cell(nn.Module):
  ...
  def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev, mask=None):
  ...
  def forward(self, s0, s1, weights):
    ...
    return torch.cat(states[-self._multiplier:], dim=1) # output is the concatenation, not sum


class MaskedNetwork(nn.Module):

  def __init__(self, C, num_classes, layers, criterion, mask_normal, mask_reduce, steps=4, multiplier=4, stem_multiplier=3):
    ...
    for i in range(layers):
      ...
      if reduction:
        cell = Cell(steps, multiplier, C_prev_prev, C_prev, C_curr, reduction, reduction_prev, mask_reduce)
      else:
        cell = Cell(steps, multiplier, C_prev_prev, C_prev, C_curr, reduction, reduction_prev, mask_normal)
  ...
  def new(self):
    model_new = MaskedNetwork(self._C, self._num_classes, self._layers, self._criterion, mask_normal=self.mask_normal, mask_reduce=self.mask_reduce)
