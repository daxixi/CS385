  ...
  for epoch in range(args.epochs):
    ...
    for client_idx in range(args.client):
      if(epoch-model_table[epoch][client_idx]>args.stale):
        ...
        current_index_normal.append(index_normal_old[len(index_normal_old)-(epoch-model_table[epoch][client_idx])][client_idx])
        current_index_reduce.append(index_reduce_old[len(index_reduce_old)-(epoch-model_table[epoch][client_idx])][client_idx])
        current_acc.append(acc_old[len(acc_old)-(epoch-model_table[epoch][client_idx])][client_idx])
    #weights
    use_models=[]
    for client_idx in range(args.client):
      if (epoch - model_table[epoch][client_idx] > args.stale):
    ...
    if epoch > args.glace_alpha:
      if(len(current_acc)>0):
        global_architect.step(current_acc,current_index_normal,current_index_reduce)
