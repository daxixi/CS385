...
def main():
    ...
    #model=VGG.VGG_19()
    model_path='eval-resnet50-20210606-153842/weights.pt'
    model.load_state_dict(torch.load(model_path))
    model = GBP(model)
    model = model.cuda()
    
    train_transform, valid_transform = utils._data_transforms_fashion(True)
    valid_data = dset.FashionMNIST('../../data', train=False, download=True, transform=valid_transform)
