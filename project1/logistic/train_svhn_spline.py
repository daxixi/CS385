...
class spline(nn.Module):
  def __init__(self):
    super().__init__()
    self.alpha=nn.Parameter(torch.Tensor(3073,1))
    self.k=nn.Parameter(torch.Tensor(3072))
    torch.nn.init.normal_(self.alpha,mean=0,std=0.01)
    torch.nn.init.normal_(self.k,mean=0,std=1)
  ...
  def forward(self,x,y):
    ...
    x=torch.mm(x,self.alpha)
    x=x.view(x.size(0))
    p1=torch.pow(y-x,2)
    p1=torch.mean(p1)
    p2=torch.pow(self.alpha[:3072],2)
    lambda_=10
    p2=torch.sum(p2)*lambda_
    loss=p1+p2
    return x,loss
...
def main():
  ...
  models=[spline() for _ in range(10)] #use 10 binary classifiers
  models=[model.cuda() for model in models]

  optimizers = [torch.optim.SGD(model.parameters(), lr=0.001,momentum=0.9) for model in models]
